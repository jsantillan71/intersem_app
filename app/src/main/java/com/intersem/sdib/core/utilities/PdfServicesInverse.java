...
public class PdfServicesInverse {
    ...
    public PdfServicesInverse(ArrayList<ServiceRequest.Fotografia> detallesArray, ServiceRequest servicio_selecionado, Context context) {
    ...
    public void iniciarPdf(){
        switch (servicio_selecionado.getReporte_subir()){
            case 1:
                name_pdf = Constants.generar_name_reporte_inverse(1, String.valueOf(this.servicio_selecionado.getNo_reporte()));
                ...
                pdf_control.addRectangleCenterText("Reporte fotografico No. " + servicio_selecionado.getNo_reporte(),150);
                break;
            case  2:
                name_pdf = Constants.generar_name_reporte_inverse(2, String.valueOf(this.servicio_selecionado.getNo_reporte()));
                pdf_control = new PdfControl(context);
                pdf_control.CreateFile(Constants.CARPETA_PDFS, name_pdf, margins_pdf_2);
                pdf_control.AddMetaData(name_pdf, "Documento 2", "SDIB");
                pdf_control.agregarEncabezadoMemoriaFotografica(servicio_selecionado.getNo_reporte());
                break;
            case 3:
                name_pdf = Constants.generar_name_reporte_inverse(3, String.valueOf(this.servicio_selecionado.getNo_reporte()));
                pdf_control = new PdfControl(context);
                pdf_control.CreateFile(Constants.CARPETA_PDFS, name_pdf, margins_pdf_3);
                pdf_control.AddMetaData(name_pdf, "Documento 3", "SDIB");
                pdf_control.agregarEncabezadoEvidenciaFotografica(servicio_selecionado.getNo_reporte());
                break;
        }
    }
